...
func write(): 
	...
	write_t = snapped(t, 0.1)
	write_x = snapped(x, 0.01)
	write_y = snapped(y, 0.01)
	
	nvalue.text = str(write_n)
	avalue.text = str(write_alpha)
	svalue.text = str(write_s)
	ovalue.text = str(write_omega)
	vvalue.text = str(write_v)
	tvalue.text = str(write_t)
